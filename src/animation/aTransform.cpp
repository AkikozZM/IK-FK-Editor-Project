...
ATransform ATransform::Inverse() const
{
	...
	mat3 R_transposed = m_rotation.Transpose();
	vec3 inv_translation = -R_transposed * m_translation;
	
	return ATransform(R_transposed, inv_translation);
}
...
vec3 ATransform::RotTrans(const vec3& vecToTransform) const
{
	...
	vec3 rotatedVector = m_rotation * vecToTransform;
	return rotatedVector + m_translation;
}
