...
{
    if (!mSkeleton || !mSkeleton->getRootNode()) {
        return;
    }
    ...
    if (updateRootXZTranslation) {
        ...
        quat rotationQuat = mMotion[root->getID()].getCachedValue(time);
        mat3 rotation;
        ...
        // other joints
        for (int i = 1; i < mSkeleton->getNumJoints(); i++) {
            AJoint* joint = mSkeleton->getJointByID(i);
            quat rotQuat = mMotion[joint->getID()].getCachedValue(time);
            mat3 rot;
            rot.FromQuaternion(rotQuat);
