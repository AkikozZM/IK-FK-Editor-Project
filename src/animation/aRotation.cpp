...
void quat::FromRotation(const mat3& rot)
{
    ...
    if (trace > 0) {
        ...
        if (rot[0][0] > rot[1][1] && rot[0][0] > rot[2][2]) {
            ...
            mQ[VX] = (rot[2][0] + rot[0][2]) / s;
            mQ[VY] = (rot[2][1] + rot[1][2]) / s;
            mQ[VZ] = 0.25f * s;
        }
    }
	Normalize();
...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
    ...
    // Handle the shortest path between quaternions
    quat q1_mod = q1;
    if (dot < 0.0)
    {
        q1_mod = -q1;
    ...
    // Near identity, just do linear interpolation
    const double THRESHOLD = 0.9995; // Near 1
    if (dot > THRESHOLD)
    {
        quat result = q0 + u * (q1_mod - q0);
        return result.Normalize();
    }
    double theta_0 = acos(dot);

    return ((sin((1 - u) * theta_0) / sin(theta_0)) * q0) +
        (sin(theta_0 * u) / sin(theta_0)) * q1_mod;
}
...
{
    //return (a + b) / 2;
    return (a + b) / (sqrt(a.SqrLength() + b.SqrLength()));
}
...
void quat::ToAxisAngle (vec3& axis, double& angleRad) const
{
    ...
    double length = sqrt(mQ[VX] * mQ[VX] + mQ[VY] * mQ[VY] + mQ[VZ] * mQ[VZ]);
    if (length < std::numeric_limits<double>::epsilon()) {
        // Handle the error, quaternion is not normalized
        axis = vec3(1, 0, 0);
        angleRad = 0;
    }
    else {
        ...
        axis = vec3(mQ[VX] * invLen, mQ[VY] * invLen, mQ[VZ] * invLen);
    }
}
...
mat3 quat::ToRotation () const
{
    ...
    float qw = mQ[VW], qx = mQ[VX], qy = mQ[VY], qz = mQ[VZ];
