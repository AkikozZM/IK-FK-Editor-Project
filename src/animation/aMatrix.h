...
// 
//  matrix declartion: 
//  aMatrix<type> A(rows, cols); where A is the name of the matrxi
//                               type is either float, double
//                               rows and cols are the number of rows and number of columns in matrix A
//        Example:  aMatrix<double> A(4,4);
//
...
#  define matrixTC  aMatrix<T>
#else
#  define MAT_TEMPLATE
#  define matrixTC  aMatrix
#  ifdef MATRIX_TYPE
...
public:
   // Constructors
   aMatrix (const matrixTC& m);
   aMatrix (size_t row = 6, size_t col = 6);

   // Destructor
   ~aMatrix ();

   // Assignment operators
   matrixTC& operator = (const matrixTC& m) _NO_THROW;

   // Unary operators
   matrixTC operator + () _NO_THROW { return *this; }
   matrixTC operator - () _NO_THROW;

   // Binary operators (SHL exposed)
//   matrixTC operator + (const matrixTC& m1, const matrixTC& m2) _THROW_MATRIX_ERROR;
//   matrixTC operator - (const matrixTC& m1, const matrixTC& m2) _THROW_MATRIX_ERROR;
//   matrixTC operator * (const matrixTC& m, const T& no) _NO_THROW;
//   matrixTC operator * (const T& no, const matrixTC& m) _NO_THROW;
//   matrixTC operator * (const matrixTC& m1, const matrixTC& m2) _THROW_MATRIX_ERROR;
//   matrixTC operator / (const matrixTC& m, const T& no) _NO_THROW;
//   matrixTC operator / (const T& no, const matrixTC& m) _THROW_MATRIX_ERROR;
//   matrixTC operator / (const matrixTC& m1, const matrixTC& m2) _THROW_MATRIX_ERROR;
//   matrixTC operator ^ (const matrixTC& m, const size_t& pow) _THROW_MATRIX_ERROR;

   // Combined assignment - calculation operators
   matrixTC& operator += (const matrixTC& m) _THROW_MATRIX_ERROR;
   matrixTC& operator -= (const matrixTC& m) _THROW_MATRIX_ERROR;
   matrixTC& operator *= (const matrixTC& m) _THROW_MATRIX_ERROR;
   matrixTC& operator *= (const T& c) _NO_THROW;
   matrixTC& operator /= (const T& c) _NO_THROW;
   matrixTC& operator ^= (const size_t& pow) _THROW_MATRIX_ERROR;
...
//   matrixTC operator ~ (const matrixTC& m) _NO_THROW;
//   matrixTC operator ! (const matrixTC m) _THROW_MATRIX_ERROR;
//   bool operator == (const matrixTC& m1, const matrixTC& m2) _NO_THROW;
//   bool operator != (const matrixTC& m1, const matrixTC& m2) _NO_THROW;
   ...
   // Utility methods
   matrixTC Solve (const matrixTC& v) const _THROW_MATRIX_ERROR;
   matrixTC Adj () _THROW_MATRIX_ERROR;
   matrixTC Inv () _THROW_MATRIX_ERROR;
...
// matrix constructor
MAT_TEMPLATE inline
matrixTC::aMatrix(size_t row, size_t col)
{
...
MAT_TEMPLATE inline
matrixTC::aMatrix (const matrixTC& m)
{
    _m = m._m;
    _m->Refcnt++;
}
...
//  reallocation method
MAT_TEMPLATE inline void 
matrixTC::reallocate (size_t row, size_t col)
{
...
MAT_TEMPLATE inline void
matrixTC::SetSize (size_t row, size_t col) _NO_THROW
{
   size_t i,j;
...
matrixTC::operator () (size_t row, size_t col) _THROW_MATRIX_ERROR
{
   if (row >= _m->Row || col >= _m->Col)
      REPORT_ERROR( "matrixTC::operator(): Index out of range!");
...
// subscript operator to get/set individual elements
MAT_TEMPLATE inline T
matrixTC::operator () (size_t row, size_t col) const _THROW_MATRIX_ERROR
{
   if (row >= _m->Row || col >= _m->Col)
      REPORT_ERROR( "matrixTC::operator(): Index out of range!");
   return _m->Val[row][col];
...
operator >> (istream& istrm, matrixTC& m)
{
...
operator << (ostream& ostrm, const matrixTC& m)
{
   for (size_t i=0; i < m.RowNo(); i++)
   {
      for (size_t j=0; j < m.ColNo(); j++)
...
operator == (const matrixTC& m1, const matrixTC& m2) _NO_THROW
{
...
matrixTC::operator += (const matrixTC& m) _THROW_MATRIX_ERROR
{
   if (_m->Row != m._m->Row || _m->Col != m._m->Col)
      REPORT_ERROR( "matrixTC::operator+= : Inconsistent matrix sizes in addition!");
   if (_m->Refcnt > 1) clone();
...
{
   if (_m->Row != m._m->Row || _m->Col != m._m->Col)
      REPORT_ERROR( "matrixTC::operator-= : Inconsistent matrix sizes in subtraction!");
   if (_m->Refcnt > 1) clone();
   for (size_t i=0; i < m._m->Row; i++)
      for (size_t j=0; j < m._m->Col; j++)
	 _m->Val[i][j] -= m._m->Val[i][j];
...
{
   if (_m->Col != m._m->Row)
      REPORT_ERROR( "matrixTC::operator*= : Inconsistent matrix sizes in multiplication!");

   matrixTC temp(_m->Row,m._m->Col);
...
MAT_TEMPLATE inline matrixTC
operator + (const matrixTC& m1, const matrixTC& m2) _THROW_MATRIX_ERROR
{
   matrixTC temp = m1;
   temp += m2;
   return temp;
...
MAT_TEMPLATE inline matrixTC
operator * (const matrixTC& m, const T& no) _NO_THROW
{
   matrixTC temp = m;
   temp *= no;
   return temp;
...
// binary scalar multiplication operator
MAT_TEMPLATE inline matrixTC
operator * (const T& no, const matrixTC& m) _NO_THROW
{
   return (m * no);
}
...
// binary scalar division operator
MAT_TEMPLATE inline matrixTC
operator / (const T& no, const matrixTC& m) _THROW_MATRIX_ERROR
{
    return (!m * no);
}
...
MAT_TEMPLATE inline matrixTC
operator ^ (const matrixTC& m, const size_t& pow) _THROW_MATRIX_ERROR
{
   matrixTC temp = m;
   temp ^= pow;
   return temp;
...
MAT_TEMPLATE inline matrixTC
operator ~ (const matrixTC& m) _NO_THROW
{
   matrixTC temp(m.ColNo(),m.RowNo());
...
MAT_TEMPLATE inline matrixTC
operator ! (const matrixTC m) _THROW_MATRIX_ERROR
{
   matrixTC temp = m;
   return temp.Inv();
}
...
matrixTC::Inv () _THROW_MATRIX_ERROR
{
   ...
   if (_m->Row != _m->Col)
      REPORT_ERROR( "matrixTC::operator!: Inversion of a non-square matrix");
   ...
   for (k=0; k < _m->Row; k++)
   {
      int indx = pivot(k);
      if (indx == -1)
	      REPORT_ERROR( "matrixTC::operator!: Inversion of a singular matrix");
...
matrixTC::Solve (const matrixTC& v) const _THROW_MATRIX_ERROR
{
   ...
   if (!(_m->Row == _m->Col && _m->Col == v._m->Row))
      REPORT_ERROR( "matrixTC::Solve():Inconsistent matrices!");

   matrixTC temp(_m->Row,_m->Col+v._m->Col);
   for (i=0; i < _m->Row; i++)
   {
      for (j=0; j < _m->Col; j++)
	 temp._m->Val[i][j] = _m->Val[i][j];
   ...
   for (k=0; k < _m->Row; k++)
   {
      ...
      if (indx == -1)
	 REPORT_ERROR( "matrixTC::Solve(): Singular matrix!");
...
MAT_TEMPLATE inline void
matrixTC::Null (const size_t& row, const size_t& col) _NO_THROW
{
    if (row != _m->Row || col != _m->Col)
	realloc( row,col);
...
MAT_TEMPLATE inline void
matrixTC::Unit (const size_t& row) _NO_THROW
{
...
MAT_TEMPLATE inline T
matrixTC::Det () const _THROW_MATRIX_ERROR
{
   size_t i,j,k;
   T piv,detVal = T(1);

   if (_m->Row != _m->Col)
      REPORT_ERROR( "matrixTC::Det(): Determinant a non-square matrix!");
...
MAT_TEMPLATE inline T
matrixTC::Cofact (size_t row, size_t col) _THROW_MATRIX_ERROR
{
   size_t i,i1,j,j1;

   if (_m->Row != _m->Col)
      REPORT_ERROR( "matrixTC::Cofact(): Cofactor of a non-square matrix!");

   if (row > _m->Row || col > _m->Col)
      REPORT_ERROR( "matrixTC::Cofact(): Index out of range!");

   matrixTC temp (_m->Row-1,_m->Col-1);
...
{
   if (_m->Row != _m->Col)
      REPORT_ERROR( "matrixTC::Adj(): Adjoin of a non-square matrix.");
...
MAT_TEMPLATE inline bool
matrixTC::IsSymmetric () _NO_THROW
{
   if (_m->Row != _m->Col)
      return false;
   for (size_t i=0; i < _m->Row; i++)
...
MAT_TEMPLATE inline bool
matrixTC::IsSkewSymmetric () _NO_THROW
{
   if (_m->Row != _m->Col)
      return false;
   for (size_t i=0; i < _m->Row; i++)
...
MAT_TEMPLATE inline bool
matrixTC::IsUpperTriangular () _NO_THROW
{
   if (_m->Row != _m->Col)
      return false;
   for (size_t i=1; i < _m->Row; i++)
...
MAT_TEMPLATE inline bool
matrixTC::IsLowerTriangular () _NO_THROW
{
   if (_m->Row != _m->Col)
      return false;
...
MAT_TEMPLATE inline void
matrixTC::ValIn(T *mat) _NO_THROW
{
	if (_m->Refcnt > 1) clone();   
	for (size_t i=0; i < _m->Row; i++)
		for (size_t j=0; j < _m->Col; j++) {
...
MAT_TEMPLATE inline void
matrixTC::ValIn2(T *mat) _NO_THROW
{
...
typedef aMatrix<double> CMatrix;
